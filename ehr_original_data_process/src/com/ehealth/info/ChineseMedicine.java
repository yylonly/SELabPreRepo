...
public class ChineseMedicine {
	...
	public String toString(){
		...
		if(this.biasList == null || this.biasList.size() == 0){
			// 没有别名
			result = "name:" + this.nameString + " number:" + this.numberString + " " + this.unitString;
		}else{
			// 有一个或多个别名
			result = "name:" + this.nameString;
			int count = biasList.size();
			for(int i = 0; i < count;i++){
				result += "bias" + (i + 1) + ":" + biasList.get(i);
			}
			result += " number:" + this.numberString + " " + this.unitString;
			return result;
		}
